...
app.post('/api/approve', verifyToken, async (req, res) => {
  try {
    ...
    if (userRole !== 'resident') {
      return res.status(403).json({ error: 'Only residents can approve visitors' });
    }
    if (!userHouseholdId) {
      return res.status(403).json({ error: 'Resident must have household assigned' });
    }
    ...
    if (visitor.householdId !== userHouseholdId) {
      return res.status(403).json({ error: 'Can only approve visitors for your own household' });
    }
    if (visitor.status !== 'pending') {
      return res.status(400).json({ error: `Cannot approve visitor with status: ${visitor.status}` });
    }
    ...
    res.json({ success: true, message: `${visitor.name} has been approved` });
  } catch (error) {
...
app.post('/api/deny', verifyToken, async (req, res) => {
  try {
    ...
    if (userRole !== 'resident') {
      return res.status(403).json({ error: 'Only residents can deny visitors' });
    }
    if (!userHouseholdId) {
      return res.status(403).json({ error: 'Resident must have household assigned' });
    }
    ...
    const visitor = visitorDoc.data();
    if (visitor.householdId !== userHouseholdId) {
      return res.status(403).json({ error: 'Can only deny visitors for your own household' });
    }
    if (visitor.status !== 'pending') {
      return res.status(400).json({ error: 'Can only deny pending visitors' });
    }
    await visitorRef.update({
      status: 'denied',
    ...
    await logEvent('denial', req.user.uid, { visitorId, visitorName: visitor.name, householdId: visitor.householdId || 'N/A', reason });
    await sendNotification(visitor.createdBy, '❌ Visitor Request Denied', `Your request for ${visitor.name} has been denied. Reason: ${reason}`, { type: 'denial', visitorId, reason });
    res.json({ success: true, message: `${visitor.name} has been denied` });
  } catch (error) {
...
// Guard and admin checkin visitor
app.post('/api/checkin', verifyToken, async (req, res) => {
  try {
    ...
    if (visitor.status !== 'approved') {
      return res.status(400).json({ error: 'Visitor must be approved first' });
    }
    await visitorRef.update({
      status: 'checked_in',
...
// Guard and admin checkout visitor
app.post('/api/checkout', verifyToken, async (req, res) => {
  try {
    ...
    if (visitor.status !== 'checked_in') {
      return res.status(400).json({ error: `Cannot check-out visitor with status: ${visitor.status}. Visitor must be checked in first.` });
    }
    await visitorRef.update({
      status: 'checked_out',
...
app.post('/api/admin/remove-user', verifyToken, async (req, res) => {
  try {
    const adminRole = req.user.role;
    ...
    await logEvent('user_removed', req.user.uid, { removedUserId: userId });
...
app.post('/api/chat', verifyToken, async (req, res) => {
  try {
    ...
    const userRole = req.user.role;
    const visitorsSnapshot = await db.collection('visitors').orderBy('createdAt', 'desc').limit(20).get();
    ...
    const tools = [
      {
        ...
        function: {
          ...
          description: 'Approve a pending visitor (resident only)',
          parameters: {
            type: 'object',
            properties: { visitorId: { type: 'string', description: 'The ID of the visitor to approve' } },
            required: ['visitorId']
          }
        }
      },
      {
        ...
        function: {
          name: 'deny_visitor',
          description: 'Deny a pending visitor (resident only)',
          parameters: {
            type: 'object',
            properties: {
      ...
      },
      {
        ...
        function: {
          ...
          parameters: {
            type: 'object',
            properties: { visitorId: { type: 'string', description: 'The ID of the visitor to check in' } },
            required: ['visitorId']
          }
        }
      },
      {
        ...
        function: {
          ...
          parameters: {
            type: 'object',
            properties: { visitorId: { type: 'string', description: 'The ID of the visitor to check out' } },
            required: ['visitorId']
          }
        }
      },
